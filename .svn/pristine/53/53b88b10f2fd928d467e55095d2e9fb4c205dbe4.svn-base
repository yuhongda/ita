...
public class oAuthSina : oAuthBase
{
    ...
    public string appSecret
    {
        get
        {
            ...
            {
                _appSecret = "b0435047babdef0b996fbd69908074a4";
            }
            return _appSecret;
        }
    ...
    public string oAuthWebRequest(Method method, string url, string postData)
    {
        ...
        if (method == Method.GET && url.IndexOf("?") > 0)
            url += postData.TrimStart('&');
        Uri uri = new Uri(url);
        ...
        if (method == Method.POST || method == Method.GET)
            ret = _WebRequest(method, outUrl + querystring, postData);// + ((method == Method.GET && url.IndexOf("?") > 0) ? postData : string.Empty)
        return ret;
    }
